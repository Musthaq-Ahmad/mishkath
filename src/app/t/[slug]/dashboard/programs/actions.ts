...
export async function createProgram(
  ...
  revalidatePath(tenantPath(slug, "/dashboard/programs"));
  revalidatePath(tenantPath(slug, "/dashboard"));
  return undefined;
}
...
export async function publishResults(programId: string): Promise<{ message?: string } | undefined> {
  ...
  revalidatePath(tenantPath(slug, "/dashboard/results"));
...
// in the same program (see supabase/migrations/20260829041027_group_multiple_teams.sql),
...
// supabase/migrations/20260829041027_group_multiple_teams.sql), so a team entry can
...
export async function addParticipant(
  ...
  revalidatePath(tenantPath(slug, `/dashboard/programs/${programId}`));
  revalidatePath(tenantPath(slug, `/dashboard/students/${studentId}`));
  return undefined;
}
...
export async function addProgramJudge(
  ...
  revalidatePath(tenantPath(slug, `/dashboard/programs/${programId}`));
  revalidatePath(tenantPath(slug, `/dashboard/programs/${programId}/scoresheet`));
  return undefined;
}
