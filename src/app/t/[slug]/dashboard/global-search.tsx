...
async function runSearch(query: string, tenantSlug: string): Promise<SearchResult[]> {
  ...
  return [
    ...(students ?? []).map((s) => ({
      ...
      label: s.name,
      sublabel: "Student",
      href: `/t/${tenantSlug}/dashboard/students?q=${encodeURIComponent(s.name)}`,
      icon: "school",
    })),
    ...(programs ?? []).map((p) => ({
      id: `program-${p.id}`,
      label: p.name,
      sublabel: "Program",
      href: `/t/${tenantSlug}/dashboard/programs/${p.id}`,
      icon: "calendar_month",
    })),
    ...(groups ?? []).map((g) => ({
      id: `group-${g.id}`,
      label: g.name,
      sublabel: "Group",
      href: `/t/${tenantSlug}/dashboard/groups?q=${encodeURIComponent(g.name)}`,
      icon: "groups",
    })),
  ];
}
...
export function GlobalSearch({ tenantSlug }: { tenantSlug: string }) {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      setLoading(true);
      const found = await runSearch(trimmed, tenantSlug);
      setResults(found);
      setLoading(false);
    }, 250);
