...
export async function createStudent(
  ...
  const { error } = await supabase.from("students").insert({
    ...
    // on division + group (see supabase/migrations/20260808102510_student_chest_numbers.sql
    // and 20260808102512_students_programs_expansion.sql).
  });
  ...
  revalidatePath(tenantPath(slug, "/dashboard/students"));
  return undefined;
}
